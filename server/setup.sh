...
sudo cp "$APP_DIR/server/safeview.service"  /etc/systemd/system/
sudo cp "$APP_DIR/server/mediamtx.service" /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable safeview mediamtx
...
sudo cp "$APP_DIR/server/nginx.conf" /etc/nginx/sites-available/safeview
sudo ln -sf /etc/nginx/sites-available/safeview /etc/nginx/sites-enabled/safeview
...
echo "    sudo sed -i 's/your-name/실제서브도메인/g' /etc/nginx/sites-available/safeview"
...
echo ""
echo " 3. 서비스 시작"
echo "    sudo systemctl start mediamtx safeview"
echo "======================================"
